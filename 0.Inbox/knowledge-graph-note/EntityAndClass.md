...
* 分类方式是不唯一的，比如说jquery框架，它使用js语言编写，用于WebUI操作，毫无疑问jquery可以作为`框架`的一个实体，详细一点划分也可以作为`js框架`的一个实体，也可以作为`UI框架`的一个实体，jquery自身也是一个复杂概念的集合，里面提供有些方法，有自己的概念（如选择器）。要更详细的描述应该描述为`JS语言编写的UI框架`或者`操作UI的JS框架`，我这里想表达的是这种**分类方式是无限的**，刚才举例的UI框架和按语言划分框架都是现实常见的分类手段，且各种分类方式下都可以有大量的数据。可见**以数据集内元素的多少来判断更应该划分成什么是不可行的**。
  ...
  生物学的分类方式也经历了数次的演化，从2界说到5界说。但不管如何演化，分类肯定是有分类的标准和依据，只要标准是明确的，无所谓有多少种分类方法，但也可以发现一个有意义的分类方式可以反应不同实体的异同的。
* 实质上关于人的分类也是有分歧的，如社会人、法人、经济人，为了避免分歧不同学说都明确了自己学说内人的概念。
* 如果时间旅行成为现实，如何定义一个人的实体也会发生改变。
* 不同分类方式包含的信息量是不同的，因为实体数量也不同。但不能以层级数量进行比较信息量。
* 比如说方法是函数，但是函数不是方法。 ng中组件是指令，指令不是组件。
* 假设分类是由层级的，且可以交叉的。一个实体可以被划分到多种分类上。分类和实体没有必然关系，是帮助理解和区分实体的概念。
